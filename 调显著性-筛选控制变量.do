//////////////////////显著性调节////////////////////////
*请看说明，已经尽最大程度注释，说明也很详尽，代码稳定可靠，不存在运行不成功。
...
*- 设定显著性水平（数据质量样本一般可以改成0.1  or 0.05）
global significance 0.01
*- 设定变量
*- 设置被解释变量  y
global dependentVariable 	y     //y修改成自己的被解释变量
*- 设置解释变量   x1
global independentVariable 	x1    //x1修改成自己的解释变量
*- 设置控制变量   z1---z8 
global controlVariables 	z1   z2 z3 z4 z5 z6 z7 z8    //z1---z8 修改成自己的控制变量，要加全




*=================================================
*第二步 （根据自己模型修改，编写属于自己的模型）
*=================================================



*- 模型可以自己修改或者添加要素 比如加控制时间、异方差、聚类 都可在下方的模型后面加  如果需要必不可少的控制变量，直接加在reg中即可比如(模型不可缺少Z1 Z3，在模型最后加上即可)如下列实例所示)
*举例说明
**********************************************************************
*若自己的模型是reg，跑（1）
reg $dependentVariable $independentVariable `tuple`i''  z1 z3,r //模型（1）

*若自己的模型是xtreg，跑（2）
xtreg $dependentVariable $independentVariable `tuple`i''  //模型（2）

*若自己的模型是xi:reg，跑（3）
xi:reg $dependentVariable $independentVariable `tuple`i''  //模型（3）

*若自己的模型是reghdfe，跑（4）
reghdfe $dependentVariable $independentVariable `tuple`i'' , absorb(year  ) ///模型（4）  吸收了年份效应（根据自己模型改）

*若自己的模型是logit，跑（5）
logit $dependentVariable $independentVariable `tuple`i''  //模型（5） 

*若自己的模型是xttobit，跑（6）
xttobit $dependentVariable $independentVariable `tuple`i'' , ll(0)  nolog tobit ///模型（6） 

*若自己的模型是probit，跑（7）
probit $dependentVariable $independentVariable `tuple`i'' //模型（7） 
...
*以下不要动，运行，直接看结果就行，结果看表（list controlVariableSet tOfIndX tValue rSq）自己找自己想要的组合！
gen degreeOfFreedom = .
gen tValue = .
gen bOfIndX = .
gen seOfIndX = .
...
forvalues i = 1/`ntuples' {
	reg $dependentVariable $independentVariable `tuple`i'' 
...
*   按以上三步走，即可
*提示：所有组合请看 list controlVariableSet tOfIndX tValue rSq的目录
...
cd E:\stata16\如何将WIND数据库下载的数据导入stata\wind数据库原始数据和调整后的数据
clear
...
gen id=real(substr(stkcd,1,6))
gen year=real(time)
order id year
tsset id year
rename 调整合适的格式满足导入stata 公司属性
rename id 股票代码
rename year 年份
rename comp 公司名称
order 股票代码 年份 公司属性
...
gen a=real(substr(accper,6,2))
drop if a!=12
drop  accper a 
// 将accper表示的年月日转换为年份，且格式转换为数值型
...
//  生成新的变量，且对老变量a001000000取ln

*(4)给变量添加标签
label var year 年度
// 给year添加标签年度，以便于在stata中读懂每个英文的含义

*(5)生成行业、年度虚拟变量 
tab Industry,gen(indu)  //生成行业虚拟变量
drop indu1     //为了避免共线性，删掉indu1
tab year,gen(time)   //生成年度虚拟变量
drop time1     //为了避免共线性，删掉time1 
// 作用——控制行业和年度效应   
...
merge 1:1 id year using 行业代码与公司年度对应至2020.dta
keep if _merge==1 | _merge==3
drop _merge
//使用 merge 1:1 匹配数据  1:1指的是1对1匹配;id year 对应的是匹配变量,id是股票代码，year是年份;“行业代码与公司年度对应至2020.dta”是匹配数据的名称
...
sum2docx  变量1 变量2 变量3 using  C:\Users\我的论文.docx,replace stats(N mean(%9.4f) sd min(%9.2f) median(%9.2f) max(%9.2f))  title("Table 2: Summary Statistics")
//该代码直接输出论文格式，简单调整即可
...
reg y x x1 x2 x3  
*论文跑数据形式
xi:reg Patent ESG Age  i.year i.ind
est store a1
esttab  a1  using 主变量回归结果.rtf,replace nogap ar2 b(%6.4f) t(%6.4f) star(* 0.1 ** 0.05 *** 0.01)  
*=============================
*        面板数据
*=============================
*基本形式
xtset id year
xtreg y x x1 x2 x3,fe //固定效应（fe）
xtreg y x x1 x2 x3,re //随机效应（re）
*论文跑数据形式（以固定效应模型为例，下面模型较多，就不一一举例，也可参考论文跑数据形式）
xi:xtreg Patent ESG Age  i.year i.ind,fe 
est store fe_1
esttab fe_1 using 固定效应方法.rtf,replace b(%6.4f) t(%6.4f) nogap ar2 star(* 0.1 ** 0.05 *** 0.01) 

*=============================
*      其二，分位数回归
*=============================
sqreg y x1 x2 x3 ,q( .1 .25 .5 .75 .9) 
//分别为0.1分位  0.25分位  0.5  0.75  0.9，可根据研究问题自行调整
...
*      其六，系统GMM
*=============================
xtabond2 y L.y x x1 x2 x3, iv(x1 x2 x3) gmm(L.y  L.(x), lag(1 2) c) robust twostep

*=============================
*  其七，双重差分模型（DID） 
*=============================
xi:xtreg 被解释变量 c.Ifexpgro#c.Iftime 控制变量,fe cl(id)
est store m1
esttab  m1 using 双重差分模型主检验.rtf,replace  ar2 b(%6.4f) t(%6.4f) star(* 0.1 ** 0.05 *** 0.01)  
...
xtreg y x x1 x2 x3,fe
est store  fe
hausman fe re    // y为被解释变量，x为解释变量，x1、x2及x3为控制变量；
//最终，选择标准为显著性在0.1及其以下选择固定效应，否则选择随机效应。

*=============================
*   其九，时间固定效应检验
*=============================
xi:xtreg y x x1 x2 x3 i.year,fe 
*或者
xtreg y x x1 x2 x3 i.year,fe 
*=============================
*   其十，个体固定效应检验
*=============================
xtreg y x x1 x2 x3 i.id,fe 
*=============================
*   其十一，行业固定效应检验
*=============================
xi:xtreg y x x1 x2 x3 i.industry,fe
*或者
xtreg y x x1 x2 x3 c.industry,fe
...
reg y x m x1 x2 x3      //(2):  m为调节变量
reg y x m x*m x1 x2 x3  //(3):  (1)中的x要显著，(3)中的x和m的乘积项要显著，则存在调节效应
...
*第二种方法即Sobel-Goodman检验
 sgmediation y, mv(x) iv(Z) cv(x1 x2 x3)   // 其中，y为被解释变量，x为解释变量，x1 x2 x3为控制变量，Z为中介变量，Indirect effect:中介效应，Direct  effect:直接效应
 
*第三种方法即Bootstrap检验
bootstrap r(ind_eff) r(dir_eff), reps(1000): sgmediation y, mv(x) iv(Z) cv(x1 x2 x3) //y为被解释变量，x为解释变量，x1 x2 x3为控制变量，Z为中介变量，计算间接效应ind_eff（_bs_1）、直接效应dir_eff（_bs_2）
...
/***Boostrap固定效应检验***/
tab 时间,gen(dt时间1)  //生成虚拟变量
tab 地区,gen(dt地区1)
sgmediation Y,mv(中介变量) iv(解释变量) cv(控制变量 dt时间1* dt地区1*)
bootstrap r(ind_eff) r(dir_eff),reps(1000) :sgmediation Y,mv(中介变量) iv(解释变量) cv(控制变量 dt时间1* dt地区1*)
estat bootstrap,percentile bc
...
*分组，东=1，中=2，西=3，东北=4
gen region = 1
replace region = 2 if id ==4 | id ==12 | id ==14 |id ==16 |  id ==17 | id ==18 
replace region = 3 if id ==5 | id ==20 | id ==22 |id ==23 |  id ==24 | id ==25 |id ==26 | id ==27 | id ==28 |id ==29 |  id ==30
replace region = 4 if id ==6 | id ==7 | id ==8
*单位根检验
*（短面板）
*法一
xtunitroot ht  y,demean
xtunitroot ht   x,demean
xtunitroot ht    z,demean
*法二
xtunitroot ips  y,demean
xtunitroot ips   x,demean
xtunitroot ips   z,demean
*（长面板）
xtunitroot llc  y
xtunitroot llc  x
xtunitroot llc  z
...
*差分再检验
gen dy=d.y
xtunitroot ht  dy,demean
gen dx=d.x
gen dz=d.z
xtunitroot ht   dx,demean
xtunitroot ht    dz,demean
*一阶差分后所有数据都不存在单位根，进行协整检验

*协整检验
*3种检验方法
xtcointtest kao y x z
*kao检验最严格，看p值，小于0.05则存在协整关系
xtcointtest pedroni y x z ,trend
*pedroni检验较为灵活，允许不同面板单位有不同的协整向量与残差自回归系数，在加上趋势项后进行检验。
*结果显示：三种统计量的P值均小于0.05，因此拒绝原假设，即变量之间存在协整关系。
*因此可以用原数据进行建模。
xtcointtest westerlund y x z,trend
*westerlund检验，看p值，小于0.05则存在协整关系

*本文采取pedroni检验。

*建立PVAR模型
*PVAR2已内置helm，不需要再单独去除个体固定效应

*确定滞后阶数，用原数据
pvar2 y x z ,lag(4) soc
*AIC、BIC、HQIC都指向2，因此滞后阶数为2阶

*重新回归，滞后阶为2
pvar2 y x z ,lag(2)
est tab, star(0.01, 0.05,0.1)
*查看1%、5%、10%水平下系数的显著性
...
*脉冲响应及方差分解
*法一：连玉君的方法（该命令在某些情况下会失效，无从得知原因，若失效，请用第二种方法，该方法内置了GMM，将会自动实现）：
pvar2 y x z , lag(2) gmm impulse monte 200 decomp(20)
*不知道为何脉冲只能做6期，monte 200是蒙特卡洛检验重复200次，一般默认200.  decomp(20)表示方差分解，代表分解20期

*法二：GMM已内置，会自动基于GMM计算
*脉冲响应（10期）
pvar2 y x z ,lag(2)  irf(10)
*方差分解（10期）
pvar2 y x z ,lag(2) irf(10) nograph decomp(10)
...
est store m1
est store m2
outreg2[m1 m2] using test1.doc,replace
...
//【序号，平衡面板】
//生成序号的数据从小至大
set obs'=31*21'//一共31个省市，21年的数据
egen id=seq(),b(10) //b（*10*）一共多少年数据就写几
bysort id:gen year=_n+2000// _n+（*2000*）填初始年份
...
//【用excel。序号，非平衡面板】
//生成序号重视个体对应的数值
//excel:=if(a3=a2,d2,d2+1)

/*一：数据初步处理*/
xtset code year  //【设定面板or非面板数据】

winsor2 y x1 x2 x3 x4 x5,replace cuts(1 99)
//缩尾处理，此命令的含义是先找到各个变量的1%、99%所对应的分位数，同时对小于和大于 1%及 99%的数据替换成分位数所对应的变量数值。
...
3.2 字符串和数字之间的转换
在转换之前，先使用 description 命令查看哪些变量是字符串，哪些是数字。如果有包含数字的字符串变量，将它们转换为数字的一个简单方法是使用 destring 命令，tostring 命令的方向正好相反。例如，如果你有一个名为 year，month，day 的字符串变量，并且字符串实际上包含数字，可以将它们转换为数字，具体命令如下所示                       //
...
类似地，如果想要在两个变量之间创建交互项，分别是 age (一个连续变量) 和 male (一个 0-1 虚拟变量)，在回归中纳入 c.age#i.male 即产生了交互作用。c. 在 age 前面表示它是一个连续变量，而 i. 在 male 前面表示该变量为 0-1 虚拟变量。使用 ## 可添加完整的交互，例如 c.age##i.male 意味着年龄、性别、年龄与性别的交互。
...
estimates table model1 model2 model3, se                       //se 选项可以为输出的回归表格中包含标准误；
estimates table model1 model2 model3, star                       //star 选项可以为输出的回归表格中的显著性水平添加星号，然而该命令不允许将 star 和 se 选项组合起来。如果希望将二者进行组合，可以使用附加命令；
...
5.2 使用附加命令输出表格
如果你需要找到可用的附加命令，可以使用 Stata 的帮助菜单和搜索选择来搜索互联网上的资源，也可以尝试使用 ssc 命令。输出回归结果表格常用的附加命令是 eststo、estout、esttab、estout 等，通过命令语句 ssc install estout, replace 就可以永久安装上述输出表格的相关命令。
...
regress yvar xvars i.entity i.time, vce(robust)                       //将因变量回归到解释变量和个体、时间的虚拟变量上，i.entity 和 i.time 表示创建个体固定效应和时间固定效应。
...
8. 工具变量回归
8.1 两阶段最小二乘法
ivregress 2sls yvar exogXVarlist (endogXVarlist = otherInstruments), vce(robust)                       //因变量 yvar 对自变量 exogXVarlist 和 endogXVarlist 的两阶段最小二乘回归，假设 endogXVarlist 中的变量是内生的，otherInstruments 为该变量的工具变量；
...
estat overid                       //在 ivregress 或 ivreg2 命令之后执行过度识别检验，需要安装第三方 overid 命令；
estat firststage                       //在 ivregress 或 ivreg2 命令之后，输出第一个阶段回归结果，如果  就可以判断为弱工具变量；
...
8.5 其他工具变量回归
对于 Probit 和 Logit 模型，可用 ivprobit 和 ivlogit 命令。对于 Tobit 模型，可用 ivtobit 命令。此外，一般的非线性 GMM 模型也可以使用 GMM 命令进行估计。
...
regress y L.y L.x1 L.x2, vce(robust)                       //对 y、y 的滞后一期、x 的滞后 1 期和 2 期进行回归，使用稳健标准误。
...
*如果要在回归结果表中包含 BIC 和 AIC 值，你可以使用 eststo 和 esttab 命令，如下述三行命令所示 
...
*估计一个 SUR 模型，corr 选项将显示残差的交叉方程相关矩阵，以及对误差项在方程之间进行具有零协方差的零假设的检验。
...
xtgls  gdp invest culture sci health admin techno, panels(correlated)
*修正依横截面而变化的异方差
xtgls  gdp invest culture sci health admin techno, panels(hetero) corr(ar1)
*修正异方差和一阶序列相关ar(1)
